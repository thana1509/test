...
const getBreedingData = (req, res, next) => {
    ...
    db.getConnection(function (err, connection) {
        if (err) {
            ...
            connection.query(`SELECT breedingID, breedingName, status, typeName, breedingDetail, date FROM breeding INNER JOIN animaltype on breeding.typeID = animaltype.typeID WHERE typename = '${typeID}'`, function (error, result) {
                connection.release()
                if (error) {
